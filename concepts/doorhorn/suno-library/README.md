...
Do not prompt with artist names, band names or "in the style of" (none appear in this library), and do not build anything that lets users generate or remix music in the app. Disclose plainly in About: "Every line written by a person. Songs produced with Suno from those lyrics." Treat the vendor situation as live: the Sony and UMG suit filed 18 to 19 September 2026 and the Munich GEMA ruling could change model availability, which is why the section maps and lyrics carry the value and the audio is a replaceable layer, and why the station ID, liftoff fanfare and launch melody are to be re-recorded with a human musician within the first year.

## 8. Controls the digest does not confirm

Treat these as things to test on the first session rather than as facts: whether Covers keep tempo and key; whether Extend keeps tempo; whether the model honours a requested key at all (check with a tuner, and pitch-shift bank clips by at most two semitones if a flavor lands elsewhere); whether Sounds mode can make a six-second musical fanfare or foley such as a clothespin; the Sounds credit cost and whether Sounds output counts as a download; whether a music-free spoken-word generation (entry 52) is possible; the exact Persona menu wording; and what v6-wild changes. Where an entry depends on one of these, its tips give a fallback.
