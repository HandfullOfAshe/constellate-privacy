...
**Why 128 for the whole daytime family.** The launch nests the two-minute teeth song inside its WAKE phase, so the launch and the Request Line must share a grid or the nest needs a tempo change. At 128 BPM every 30 seconds is exactly 16 bars and every minute is exactly 32 bars, so all five launch durations (8, 10, 12, 15, 20 minutes) and all six Request Line lengths (1:00, 1:30, 2:00, 3:00, 5:00, 10:00) are whole numbers of 16-bar blocks, and the teeth quadrant switches at 0:30, 1:00 and 1:30 fall on phrase downbeats. A bar is 90,000 samples at 48 kHz, an integer, so trims never accumulate fractional-sample drift. 120 BPM was rejected because 30 seconds is then 15 bars (no phrase symmetry for the quadrants) and 10- and 15-minute launches are not multiples of an 8-bar phrase. 128 is inside the natural range of a quick march, disco, surf and bluegrass, and a medium swing for big band; the calm layer plays half-time feel at 6:58 a.m. so the tempo reads as brisk, not frantic.

**Why 80 → 60 for Re-entry.** WINNER.md asks for a descent of roughly 20 BPM over twenty minutes ending on a buzzer. 80 to 60 is exactly 20, ends at one beat per second (resting pulse), and every step tempo divides 11,520,000 (240 × 48,000) so each segment's bar is an integer sample count. The step tempos were chosen so each segment is a whole number of bars (section 7).

**Why 96 for End Credits.** Sixty seconds is exactly 24 bars, which splits into 2 + 4 + 12 + 4 + 2 bar slots (section 8). It is also a comfortable late-night swing tempo for Bunny's lane.

**Keys.** Launches begin in F major and modulate up a whole step to G major at the DRESS key change; the MANIFEST bridge, the DOOR chorus, the fanfare, every Gus sung line, Re-entry and the End Credits are all in G major. One house key means every runtime-assembled sung line fits every bed it can land on. Request Line songs are monolithic and may sit in any key, recorded in their map. Key is a prompt hint in Suno song generation, not a control, so it is verified in QA (section 14).
...
Fixed anchors that the household learns: the horn entry is always the downbeat of DOOR; the manifest bridge always begins at MANIFEST; the teeth nest always begins at bar 41. Section 4's worked example gives the 12-minute bar numbers.

**Daily variation.** An arrangement is chosen by a seeded generator (seed = local date + household id) from the legal set: order of A/B/BREATH repeats in WAKE, DR count split against DOOR count within the duration budget (the table above is the default balance; ±32 bars may shift between DRESS and DOOR), layer states per repeat (a repeated A may be calm or medium), one of four authored 1-bar drum fills at each section join, walk-up cue slots, and the flavor itself (rotation across unlocked flavors, never the same flavor two days running unless only one is unlocked). For the 12-minute launch alone the legal set has more than 200 distinct arrangements; across six flavors and cue rotation the space is in the tens of thousands. The app stores a hash of the last 60 arrangements and re-rolls on a collision, so no launch repeats exactly inside two months, and the 300th morning is a different arrangement of music the household already knows.
...
Reading the example: WAKE runs bars 1–136 (0:00–4:15) including the teeth nest at bars 41–104 (1:15–3:15) with quadrant cues every 16 bars; DRESS is three reads of the same 32-bar section (4:15–7:15) with the key change on bar 137; MANIFEST is 48 bars (7:15–8:45): four bars of lead-in, twelve 2-bar item slots, a 4-bar Hold slot, then a 16-bar roll-call window in which a solo household's four role stems auto-punch every four bars; the horns enter on bar 281 at exactly 8:45.000; three choruses and the tag run to T-0 at 12:00.000, where the fanfare sting starts. Every `ms` equals `(bar − 1) × 1875`.
...
Intensity is a layer state (calm = L_CALM; medium = L_CALM + L_MED; intense = all three), not separate audio. Role stems are silent before the manifest bridge by design: the band assembles during the roll call and T-0 is the first moment everything sounds together. L_MED and L_HOT therefore must carry enough rhythm (hand percussion, comping) for the first phases to feel alive without a kit; this is a prompt and split requirement, checked in QA.

### 4.2 Producing the stems from Suno

1. Generate the master song (section 12) and, from the same clip, run stem separation. Suno offers three modes: **Auto Split** (up to 12 stems for 50 credits, a true separation whose stems sum back to the mix), **Split from Mix** (Pro, 10 credits per stem) and **Advanced Split** (Premier only, pick from nearly 100 instruments at 20 credits per stem, which *regenerates* the stem rather than slicing the mix) (https://help.suno.com/en/articles/12702337).
2. Use **Advanced Split** for the four role stems, since it isolates named instruments (drum kit, bass, brass section, choir) cleanly and the roll call needs them clean. Because Advanced Split regenerates rather than slices, each role stem must be checked against the master for grid alignment (section 14) and, if it drifts, replaced by the corresponding Auto Split slice.
...
Tempo drift is normal in Suno output ("just like live music", https://help.suno.com/en/articles/8363457); the documented fix is Studio's Project Tempo → Manual BPM before exporting. After export, the bar-1 downbeat sample `S0` is located once on R_DRUMS (kick onset) and the same `S0` and length (208 × 90,000 = 18,720,000 samples) are applied to all eight stems, so they stay sample-aligned to each other (commands in section 12.7). A master whose bars drift more than ±15 ms from the grid by bar 208 after Manual BPM is regenerated; it is not time-stretched, because stretching eight stems separately introduces artefacts that are audible at punch-ins.

### 4.4 How the Stem Roll Call punches in

All eight stems are scheduled on separate `AVAudioPlayerNode`s from the same start sample and play continuously for the whole arrangement, the unconfirmed ones at gain 0. A confirm tap is timestamped on the engine's render clock, the next bar boundary is computed from `barSamples`, and the stem's gain ramps 0 → 1 over `tap.rampMs` (10 ms) starting exactly on that sample. Nothing is started at tap time, so there is no scheduling latency and the join is sample-exact. On AirPlay, output is delayed uniformly (up to about two seconds), so the punch-in still lands on a bar as heard; the Live Activity follows playback position, not the tap. A walk-up cue (2 bars, 3.75 s, on the `sting` bus) starts on the beat after the tap and the stem enters on the first bar boundary after the cue ends. Section reads (repeats) are `scheduleSegment` calls at bar-exact sample offsets into the same files; `fillBefore` swaps the last bar of a section for one of four 1-bar fills on R_DRUMS only if Pilot is confirmed, otherwise on L_MED's percussion.
...
**Getting an exact length out of Suno.** There is no exact-length control in Suno's own v6 interface: the v6 release notes name section editing, mashups and multi-modal inputs but no length or tempo setting (https://suno.com/release-notes/introducing-v6); the v5.5 Duration slider workflow is described as legacy (https://jackrighteous.com/en-us/blogs/guides-using-suno-ai-music-creation/suno-duration-slider-control-song-length-in-v5-5). A third-party wrapper advertises a 10 s–6 min length field for v6 and v6-wild, and even there dense arrangements overrun a short target because "Suno finishes the musical phrase it started" (https://kolbo.ai/blog/suno-v6-launch); that wrapper is not used here because commercial rights attach to permitted downloads from Suno's own service. Length is therefore controlled by writing, not by a slider:
...
If the sung cue in a take lands one bar late throughout, the map moves, not the audio: quadrant cues are allowed at bar 17 ± 1 as long as all four are consistent, because the household hears the song, not the design.
...
**Runtime assembly rules.** Bed and lines are on different buses; the bed is continuous. A line is scheduled at `slotStart − preRollSamples` and gated by a 20 ms equal-power fade at both ends on the `vocalLine` bus; two lines never overlap by more than their tails. Every line is normalized to the same short-term loudness (section 11) so the bed sits at a fixed −6 dB under the vocal bus. If the manifest has fewer than four billable roles, empty billing slots are filled with 4-bar instrumental reads of the bed (source bars 7–10). The same key, tempo and bar length across bank and bed is the entire assembly contract; a line in the wrong key fails QA and is never shipped.
...
Lanes beat voice locks because of how Suno's continuity tools behave. A Persona "recalls the identity of a Suno song" and may keep "vocal character and general delivery, genre and stylistic tendencies, instrumentation and sonic palette", but "a Persona does not lock" one exact singer or identical timbre, and "new lyrics, vocal demands, active controls and model behaviour can create drift" (https://jackrighteous.com/en-us/blogs/guides-using-suno-ai-music-creation/suno-personas-keep-the-same-voice-across-songs, rebuilt 1 Aug 2026). Voices are built around an authorized recorded singing voice and the Terms allow a Voice Model of your own voice only (https://suno.com/terms-of-service), so no character can be voice-locked except by the developer singing it. A lane, by contrast, is a written spec (instrumentation, register, tempo, key, delivery notes) that any model, any Persona and any human musician can render, so drift between takes reads as the same band on a different day, and a model retirement (Suno retires all prior models when new ones launch: https://suno.com/blog/suno-updates-tos) does not orphan a character. Each lane gets one Persona per model line as a convenience, with the lane spec as the contract. Spoken cues (Augustine's calls, Lionel's "copy", Bunny's phase cues) are not generated with Suno at all: they are recorded with a human voice actor, which is cheaper, exact to the millisecond, and adds a human-authored layer to every launch.
...
The AAC choice rests on one engineering gate (section 14): the decoded frame count reported by `AVAudioFile.length` must equal `lengthBars × barSamples + preRoll + tail` exactly for every asset. If any asset class fails, that class ships as ALAC.
...
Commercial rights attach to a *permitted download* on a paid tier; one song is one download regardless of format or stems; Pro allows 20 a month, Premier 60 a month, and Studio downloads on Premier are unlimited (https://suno.com/terms-of-service; https://help.suno.com/en/articles/13614785; https://help.suno.com/en/articles/13876865; https://suno.com/blog/suno-updates-tos). Extra downloads are about $2.99 each (https://roo.beehiiv.com/p/suno-download-prices-explained). Because the Terms' "permitted download" wording has not been tested against Studio's unlimited exports (digest open item), every shipped song is downloaded at least once through the normal library download so it is unambiguously a permitted download, and Studio exports are used for stems and WAV.
...
That is one Premier month for beta and two for launch (60 each), consistent with WINNER.md, with Studio covering all stem and WAV exports. Every download is logged (section 13).
...
1. **Tier: Premier.** Only Premier has Studio (WAV export, unlimited Studio downloads, Manual BPM), Advanced Split and 60 downloads a month; Pro has no Studio and 20 downloads (https://suno.com/pricing, fetched 24 Sept 2026, which rendered $24/month while secondary sources say $30 monthly, $24 annual; confirm at checkout). Never generate anything for the app on the free tier: free output is personal and non-commercial and cannot be licensed retroactively (https://help.suno.com/en/articles/2425729).
2. **Model: v6 (flagship).** v6 and v6-wild are paid-only; v6-mini is the free model (https://suno.com/release-notes/introducing-v6). v6-wild is excluded because its outputs are "less predictable" and this library needs repeatable tempo and structure; v6-mini is excluded because it is the free-tier model. v6 is the line Suno describes as trained on licensed partner content (https://www.digitalmusicnews.com/2026/09/22/suno-comments-umg-sony-lawsuit/), which matters for section 13.
3. **Project settings.** Custom Mode; style prompt under 1,000 characters, front-loaded with genre, vocal type, mood, instruments and the BPM number; lyrics under 5,000 characters with bracketed section tags, and about 3,000 characters as the quality sweet spot (https://hookgenius.app/learn/suno-character-limits/). For instrumental beds: Instrumental toggle on, `[Instrumental]` in the lyrics box, and "vocals" in the Exclude field (https://blakecrosley.com/guides/suno). Weirdness low, Style Influence high; change one advanced setting at a time with identical lyrics (https://moelueker.com/blog/suno-v6-guide). No Max Mode unless a take is being finalized (it doubles credit cost, same source).
...
**After.** Keep unmodified originals exactly as downloaded, hashed, alongside the CAF derivatives; never "remove, alter, obscure or circumvent any fingerprint, watermark or metadata Suno appends" (https://suno.com/terms-of-service). Loudness and trimming operate on Studio WAV exports and are ordinary post-production, not watermark removal; if Suno's fingerprint is later documented as fragile to processing, this decision is revisited. Archive `originals/`, `prompts/`, `receipts/`, `lyrics/` and `maps/` off-platform (encrypted external drive plus a second copy) because Suno has changed terms once in 2026 and suffered a large data breach (https://www.musicbusinessworldwide.com/suno-hit-with-class-action-lawsuit-over-data-breach-reportedly-affecting-55m-users/).
...
**Contingency: a Suno model or service is withdrawn.** Already-downloaded outputs keep their contractual commercial grant, so shipped audio is not pulled on a retirement alone. The live risks are litigation-driven: Sony and UMG's second suit (filed 18–19 Sept 2026, D. Mass., up to $9B over 60,202 recordings, arguing v6 is "fruit of the poisoned tree": https://www.engadget.com/2262978/sony-music-and-udio-say-sunos-new-models-still-violates-their-copyright.html) and the GEMA judgment in Munich (31 July 2026, not final: https://www.twobirds.com/en/insights/2026/germany/munich-district-court-rules-on-ai-generated-music-gema-v-suno). The plan: (1) the section maps, lyrics and lane specs are provider-neutral, so a full re-render is 12 launch songs, 24 Request Line songs, 6 Re-entry segments and the banks, about three weeks of production on any licensed provider or with session musicians; (2) the human-recorded station ID, fanfare and melody are the fallback identity; (3) a v1.x update can swap the audio bundle without touching maps or code; (4) if an injunction ever reached distributed outputs, the app ships the human-recorded assets plus a reduced library while the rest is re-rendered. Store availability by territory is reviewed if the GEMA ruling is upheld.
...
1. **Is 128 BPM too brisk at 6:58 a.m.?** The calm layer in half-time feel is the mitigation; the alternative is 96 BPM (bar = 2.5 s, 30 s = 12 bars), which keeps clean quadrants but loses 8-bar symmetry. Decide after the first flavor is heard in a kitchen.
2. **Do kit-less WAKE and DRESS phases feel thin?** The "band arrives at the roll call" design is faithful to WINNER.md, but L_MED and L_HOT may need a light "ghost kit" that a Pilot confirm replaces. Test with the reluctant partner in the room.
3. **Will Covers preserve the F → G modulation and the 208-bar structure?** Suno's own note says a cover may return the original unadapted; drift in structure is likely on some lanes. Budget assumes one retake per flavor.
4. **Does Advanced Split's regenerated stem stay on the grid?** If not, role stems fall back to Auto Split slices, which are less isolated. Unknown until tried.
5. **Do Studio exports fully satisfy "permitted download"?** Suno's FAQ says Studio downloads are unlimited on Premier, but the Terms' wording is untested; the one-library-download-per-song rule is the hedge.
6. **Are Sounds one-shots counted as downloads?** Not stated; reels are the hedge.
7. **Does `AVAudioFile` report AAC lengths without priming frames for every asset class?** If any class fails the decoded-length gate, it ships as ALAC and the size table grows by roughly 2× for that class.
8. **Vocabulary limits of the manifest bank.** Items outside the ~150-line bank get a generic sung line and the item text on the card; a "record it in your own voice" option (3 s, spoken, over the bed) is the candidate v1.1 answer.
...
10. **Pricing.** suno.com/pricing rendered $24/month for Premier on 24 Sept 2026 while secondary sources report $30 monthly ($24 annual); confirm at checkout and file the receipt.

---

### Sources referenced

Suno Terms of Service (https://suno.com/terms-of-service); Suno help: ownership (https://help.suno.com/en/articles/2416769), paid rights (https://help.suno.com/en/articles/9601665), no retroactive rights (https://help.suno.com/en/articles/2425729), distribution and attribution (https://help.suno.com/en/articles/2410177), copyright and lyrics (https://help.suno.com/en/articles/2746945), download FAQ (https://help.suno.com/en/articles/13614785), how downloads work (https://help.suno.com/en/articles/13876865), stem separation (https://help.suno.com/en/articles/12702337), Studio exports (https://help.suno.com/en/articles/8128193), tempo drift (https://help.suno.com/en/articles/8363457); Suno blog and release notes: ToS update (https://suno.com/blog/suno-updates-tos), Covers (https://suno.com/blog/covers), v6 (https://suno.com/release-notes/introducing-v6), Sounds (https://suno.com/release-notes/make-loops-and-samples-from-scratch-with-sounds), release-notes index (https://suno.com/release-notes); pricing (https://suno.com/pricing); extra-download prices (https://roo.beehiiv.com/p/suno-download-prices-explained); Personas guide (https://jackrighteous.com/en-us/blogs/guides-using-suno-ai-music-creation/suno-personas-keep-the-same-voice-across-songs); duration guide (https://jackrighteous.com/en-us/blogs/guides-using-suno-ai-music-creation/suno-duration-slider-control-song-length-in-v5-5); tempo guide (https://hookgenius.app/learn/suno-tempo-bpm-guide/); character limits (https://hookgenius.app/learn/suno-character-limits/); game-loop practice (https://hookgenius.app/learn/suno-for-game-developers/); v6 prompting (https://moelueker.com/blog/suno-v6-guide); meta tags (https://blakecrosley.com/guides/suno); third-party length control (https://kolbo.ai/blog/suno-v6-launch); Apple QA1636 (https://developer.apple.com/library/archive/qa/qa1636/_index.html); Apple guidelines (https://developer.apple.com/app-store/review/guidelines/); Apple loudness guidance (https://mastering.to/apple-music; https://www.masteringbox.com/learn/mastering-for-streaming); Copyright Office Part 2 (https://copyright.gov/ai/Copyright-and-Artificial-Intelligence-Part-2-Copyrightability-Report.pdf); Thaler (https://www.scotusblog.com/cases/thaler-v-perlmutter/); Sony/UMG v. Suno II (https://www.engadget.com/2262978/sony-music-and-udio-say-sunos-new-models-still-violates-their-copyright.html); Suno's response (https://www.digitalmusicnews.com/2026/09/22/suno-comments-umg-sony-lawsuit/); GEMA v. Suno (https://www.twobirds.com/en/insights/2026/germany/munich-district-court-rules-on-ai-generated-music-gema-v-suno); data breach (https://www.musicbusinessworldwide.com/suno-hit-with-class-action-lawsuit-over-data-breach-reportedly-affecting-55m-users/); no public API (https://www.musicbusinessworldwide.com/suno-explores-developer-api-seeking-apps-that-unlock-experiences-generative-music-makes-possible-for-the-first-time/).
